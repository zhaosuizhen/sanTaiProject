    ...
    <div class="management-top2">
      ...
      <el-col :span="6" class="management-top-aside2">
        ...
        <div class="management-top-div2">
          <div class="management-charts-title">市场数量(2019)</div>
      ...
      <el-col :span="6" class="management-top-aside2">
        ...
        <!-- <div class="management-top-div2" @click="handleCountChart"> -->
        <div class="management-top-div2">
          <div class="management-charts-title">交易品种(2019)</div>
...
import { changesInPrices, MYchangesInPrices } from '../../api/home'
export default {
  data() {
    return {
  ...
  */
  mounted() {
    ...
    this.$http
      .get(
        api.ip + "/web/cMarketTransaction/transactionData"
      )
      .then(
        res => {
    ...
    this.$http
      .get(
        api.ip + "/web/cMarketTransaction/changesInPrices?type=0"
      )
      .then(
        res => {
          console.log(res)
          ...
          this.marketData.name = nameArr.map(item => item.substring(item.indexOf('-') + 1));
          this.initMarketVegetablesChart(this.marketData);
        },
        err => {
          // this.$message.error("蔬菜价格变化数据请求失败！");
        }
      );
    //水果价格变化
    this.$http
      .get(
        api.ip + "/web/cMarketTransaction/changesInPrices?type=1"
      )
      .then(
        res => {
          console.log(res)
        ...
        err => {
          // this.$message.error("水果价格变化数据请求失败！");
        }
      );
    //肉类价格变化
    this.$http
      .get(
        api.ip + "/web/cMarketTransaction/changesInPrices?type=2"
      )
      .then(
        res => {
        ...
        err => {
          // this.$message.error("肉类价格变化数据请求失败！");
        }
      );
  ...
  methods: {
    ...
    async getChangesInPrices(){
      let res_vegetables = await changesInPrices({type:0})
      console.log(res_vegetables.data.data)
      let res_fruits = await changesInPrices({type:1})
      let res_meats = await changesInPrices({type:2})
