    ...
    <div id="qualityInspection-list-component">
        ...
        <el-row class="searchContent">
            <el-col :span='12' class="toolLeft">
                ...
                <el-button type="primary" size="small" :disabled="!allSelectionID.length" @click="flag = true" v-if='this.featureFlag !== "自定义表单"'>审核</el-button>
            ...
            <el-col :span='12' class="toolRight">
                <div class="searchCriteria">
                    ...
                    <template v-if='this.featureFlag !== "自定义表单"'>
                        <span>审核状态：</span>
                        <el-select v-model="filters.isCheck" clearable size="small" class="pickerWidth formSelect" placeholder="请选择">
                            <el-option
        ...
        <el-table highlight-current-row :data="tableData.records" @selection-change="selsChange" v-loading="listLoading" style="margin-top:80px" height="65vh">
            ...
            <el-table-column label="审核结果"  fixed="right" align="center" v-if='this.featureFlag !== "自定义表单"'>
...
import { rpManageList, rpManageDelete, rpManageAudit } from '@/utils/api/ruralEconomy/rpManage'  //农经股---藤椒经营主体
import { vegetableDevelopmentList, vegetableDevelopmentDelete, vegetableDevelopmentAudit } from '@/utils/api/ruralEconomy/vegetableDevelopment'  //农经股---蔬菜发展情况
import { rpInformationList, rpInformationDelete, rpInformationAudit } from '@/utils/api/ruralEconomy/rpInformation'  //农经股---藤椒基本信息
import { industrialDevelopmentList, industrialDevelopmentDelete, industrialDevelopmentAudit } from '@/utils/api/ruralEconomy/industrialDevelopment'  //农经股---藤椒产业发展
...
import { diseaseControlDataList, diseaseControlDataDelete, diseaseControlDataAudit } from '@/utils/api/plantProtectionStation/diseaseControlData'  //植保站---疾病防治数据
...
export default {
    ...
    methods:{
        ...
        async queryListByPage(){
            ...
            }else if(this.$route.path.indexOf("/rpManage") >= 0){
                res = await rpManageList(obj);  //农经股---藤椒经营主体  分页列表查询
            }else if(this.$route.path.indexOf("/rpInformation") >= 0){
                res = await rpInformationList(obj);  //农经股---藤椒基本信息  分页列表查询
                res.data.entitys.records.forEach(item => {
                    item.plantingArea = item.notProduction + item.productioned
                    item.yieldInfo = item.dryWeight + item.freshWeight
                    item.processingOutput = item.freshYield + item.dryYield + item.oilYield + item.keepFreshYield
                    item.salesVolume = item.salesVolume + item.drySalesVolume + item.flavoringSalesVolume
                    item.incomeOfFarmers = item.familySalesIncome + item.familyMachiningIncome + item.transportSalesIncome + item.labourIncome
                })
            }else if(this.$route.path.indexOf("/industrialDevelopment") >= 0){
                res = await industrialDevelopmentList(obj);  //农经股---藤椒产业发展  分页列表查询
                res.data.entitys.records.forEach(item => {
                    item.developmentArea2018 = (item.ajz2018DevelopmentArea + item.bdz2018DevelopmentArea + item.bqx2018DevelopmentArea + item.bqz2018DevelopmentArea +
                        item.dsx2018DevelopmentArea + item.dtz2018DevelopmentArea + item.fsz2018DevelopmentArea	+ item.ghz2018DevelopmentArea + item.gjz2018DevelopmentArea +
                        item.glx2018DevelopmentArea + item.gqz2018DevelopmentArea + item.hjz2018DevelopmentArea	+ item.hyz2018DevelopmentArea + item.jdx2018DevelopmentArea +
                        item.jfz2018DevelopmentArea + item.jg2018DevelopmentArea + item.jhz2018DevelopmentArea	+ item.jp2018DevelopmentArea + item.jsx2018DevelopmentArea +
                        item.jsz2018DevelopmentArea + item.jz2018DevelopmentArea + item.laz2018DevelopmentArea	+ item.lb2018DevelopmentArea + item.lc2018DevelopmentArea +
                        item.lcz2018DevelopmentArea + item.lj2018DevelopmentArea + item.lmz2018DevelopmentArea	+ item.ls2018DevelopmentArea + item.lsz2018DevelopmentArea +
                        item.lxinz2018DevelopmentArea + item.lxx2018DevelopmentArea + item.lxz2018DevelopmentArea	+ item.ly2018DevelopmentArea + item.qfz2018DevelopmentArea +
                        item.qlz2018DevelopmentArea + item.sax2018DevelopmentArea + item.sgx2018DevelopmentArea	+ item.slx2018DevelopmentArea + item.ssz2018DevelopmentArea +
                        item.sxs2018DevelopmentArea + item.sy2018DevelopmentArea + item.tcz2018DevelopmentArea	+ item.tsz2018DevelopmentArea + item.waz2018DevelopmentArea +
                        item.xfz2018DevelopmentArea + item.xhx2018DevelopmentArea + item.xlz2018DevelopmentArea	+ item.xpz2018DevelopmentArea + item.xsz2018DevelopmentArea +
                        item.xxx2018DevelopmentArea + item.yjz2018DevelopmentArea + item.ylz2018DevelopmentArea	+ item.ymz2018DevelopmentArea + item.ytx2018DevelopmentArea +
                        item.zhihez2018DevelopmentArea + item.zsz2018DevelopmentArea + item.ztz2018DevelopmentArea	+ item.zxinz2018DevelopmentArea + item.zxx2018DevelopmentArea).toFixed(2)

                    item.developmentArea2019 = (item.ajz2019DevelopmentArea + item.bdz2019DevelopmentArea + item.bqx2019DevelopmentArea + item.bqz2019DevelopmentArea +
                        item.dsx2019DevelopmentArea + item.dtz2019DevelopmentArea + item.fsz2019DevelopmentArea	+ item.ghz2019DevelopmentArea + item.gjz2019DevelopmentArea +
                        item.glx2019DevelopmentArea + item.gqz2019DevelopmentArea + item.hjz2019DevelopmentArea	+ item.hyz2019DevelopmentArea + item.jdx2019DevelopmentArea +
                        item.jfz2019DevelopmentArea + item.jg2019DevelopmentArea + item.jhz2019DevelopmentArea	+ item.jp2019DevelopmentArea + item.jsx2019DevelopmentArea +
                        item.jsz2019DevelopmentArea + item.jz2019DevelopmentArea + item.laz2019DevelopmentArea	+ item.lb2019DevelopmentArea + item.lc2019DevelopmentArea +
                        item.lcz2019DevelopmentArea + item.lj2019DevelopmentArea + item.lmz2019DevelopmentArea	+ item.ls2019DevelopmentArea + item.lsz2019DevelopmentArea +
                        item.lxinz2019DevelopmentArea + item.lxx2019DevelopmentArea + item.lxz2019DevelopmentArea	+ item.ly2019DevelopmentArea + item.qfz2019DevelopmentArea +
                        item.qlz2019DevelopmentArea + item.sax2019DevelopmentArea + item.sgx2019DevelopmentArea	+ item.slx2019DevelopmentArea + item.ssz2019DevelopmentArea +
                        item.sxs2019DevelopmentArea + item.sy2019DevelopmentArea + item.tcz2019DevelopmentArea	+ item.tsz2019DevelopmentArea + item.waz2019DevelopmentArea +
                        item.xfz2019DevelopmentArea + item.xhx2019DevelopmentArea + item.xlz2019DevelopmentArea	+ item.xpz2019DevelopmentArea + item.xsz2019DevelopmentArea +
                        item.xxx2019DevelopmentArea + item.yjz2019DevelopmentArea + item.ylz2019DevelopmentArea	+ item.ymz2019DevelopmentArea + item.ytx2019DevelopmentArea +
                        item.zhihez2019DevelopmentArea + item.zsz2019DevelopmentArea + item.ztz2019DevelopmentArea	+ item.zxinz2019DevelopmentArea + item.zxx2019DevelopmentArea).toFixed(2)

                    item.survivalArea = (item.ajzSurvivalArea + item.bdzSurvivalArea + item.bqxSurvivalArea + item.bqzSurvivalArea +
                        item.dsxSurvivalArea + item.dtzSurvivalArea + item.fszSurvivalArea	+ item.ghzSurvivalArea + item.gjzSurvivalArea +
                        item.glxSurvivalArea + item.gqzSurvivalArea + item.hjzSurvivalArea	+ item.hyzSurvivalArea + item.jdxSurvivalArea +
                        item.jfzSurvivalArea + item.jgSurvivalArea + item.jhzSurvivalArea	+ item.jpSurvivalArea + item.jsxSurvivalArea +
                        item.jszSurvivalArea + item.jzSurvivalArea + item.lazSurvivalArea	+ item.lbSurvivalArea + item.lcSurvivalArea +
                        item.lczSurvivalArea + item.ljSurvivalArea + item.lmzSurvivalArea	+ item.lsSurvivalArea + item.lszSurvivalArea +
                        item.lxinzSurvivalArea + item.lxxSurvivalArea + item.lxzSurvivalArea	+ item.lySurvivalArea + item.qfzSurvivalArea +
                        item.qlzSurvivalArea + item.saxSurvivalArea + item.sgxSurvivalArea	+ item.slxSurvivalArea + item.sszSurvivalArea +
                        item.sxsSurvivalArea + item.sySurvivalArea + item.tczSurvivalArea	+ item.tszSurvivalArea + item.wazSurvivalArea +
                        item.xfzSurvivalArea + item.xhxSurvivalArea + item.xlzSurvivalArea	+ item.xpzSurvivalArea + item.xszSurvivalArea +
                        item.xxxSurvivalArea + item.yjzSurvivalArea + item.ylzSurvivalArea	+ item.ymzSurvivalArea + item.ytxSurvivalArea +
                        item.zhihezSurvivalArea + item.zszSurvivalArea + item.ztzSurvivalArea	+ item.zxinzSurvivalArea + item.zxxSurvivalArea).toFixed(2)

                    item.operationArea = (item.ajzOperationArea + item.bdzOperationArea + item.bqxOperationArea + item.bqzOperationArea +
                        item.dsxOperationArea + item.dtzOperationArea + item.fszOperationArea	+ item.ghzOperationArea + item.gjzOperationArea +
                        item.glxOperationArea + item.gqzOperationArea + item.hjzOperationArea	+ item.hyzOperationArea + item.jdxOperationArea +
                        item.jfzOperationArea + item.jgOperationArea + item.jhzOperationArea	+ item.jpOperationArea + item.jsxOperationArea +
                        item.jszOperationArea + item.jzLyOperationArea + item.lazOperationArea	+ item.lbOperationArea + item.lcOperationArea +
                        item.lczOperationArea + item.ljOperationArea + item.lmzOperationArea	+ item.lsOperationArea + item.lszOperationArea +
                        item.lxinzOperationArea + item.lxxOperationArea + item.lxzOperationArea	+ item.lyOperationArea + item.qfzOperationArea +
                        item.qlzOperationArea + item.saxOperationArea + item.sgxOperationArea	+ item.slxOperationArea + item.sszOperationArea +
                        item.sxsOperationArea + item.syOperationArea + item.tczOperationArea	+ item.tszOperationArea + item.wazOperationArea +
                        item.xfzOperationArea + item.xhxOperationArea + item.xlzOperationArea	+ item.xpzOperationArea + item.xszOperationArea +
                        item.xxxOperationArea + item.yjzOperationArea + item.ylzOperationArea	+ item.ymzOperationArea + item.ytxOperationArea +
                        item.zhihezOperationArea + item.zszOperationArea + item.ztzOperationArea	+ item.zxinzOperationArea + item.zxxOperationArea).toFixed(2)
                })
            }else if(this.$route.path.indexOf("/vegetableDevelopment") >= 0){
                res = await vegetableDevelopmentList(obj);  //农经股---蔬菜发展情况  分页列表查询
                res.data.entitys.records.forEach(item => {
                    item.spcdYield = item.cncdYield * 1 + item.jjcdYiled * 1
                    item.spcdVegetablesSow = item.cncdVegetablesOccupy * 1 + item.jjcdVegetablesOccupy * 1
                })
            ...
            }else if(this.$route.path.indexOf("/diseaseControlData") >= 0){
                res = await diseaseControlDataList(obj);  //植保站---  疾病防治数据
        ...
        async deleteItems(ids){
            ...
            if(this.$route.path.indexOf("/qualityOrganicProducts") >= 0 || this.$route.path.indexOf("/doOrganicFoodEnter") >= 0){
                ...
                deletRes = await rpManageDelete(ids);  //----农经股---藤椒经营主体  删除
            }else if(this.$route.path.indexOf("/rpInformation") >= 0){
                deletRes = await rpInformationDelete(ids);  //----农经股---藤椒基本信息  删除
            }else if(this.$route.path.indexOf("/industrialDevelopment") >= 0){
                deletRes = await industrialDevelopmentDelete(ids);  //----农经股---藤椒产业发展  删除
            }else if(this.$route.path.indexOf("/vegetableDevelopment") >= 0){
                deletRes = await vegetableDevelopmentDelete(ids);  //----农经股---蔬菜发展情况  删除
            }else if(this.$route.path.indexOf("/seedEnterprise") >= 0){
                ...
                deletRes = await diseaseControlDataDelete(ids);  //----植保站=---疾病防治数据  删除
            }else if(this.$route.path.indexOf("/industryLeavePoor") >= 0){
        ...
        async auditItems (ids,status) {
            ...
            if(this.$route.path.indexOf("/qualityOrganicProducts") >= 0 || this.$route.path.indexOf("/doOrganicFoodEnter") >= 0){
                ...
                auditRes = await rpManageAudit(ids,{status});  //农经股----藤椒经营主体  审核
            }else if(this.$route.path.indexOf("/rpInformation") >= 0){
                auditRes = await rpInformationAudit(ids,{status});  //农经股----藤椒基本信息  审核
            }else if(this.$route.path.indexOf("/industrialDevelopment") >= 0){
                auditRes = await industrialDevelopmentAudit(ids,{status});  //农经股----藤椒产业发展  审核
            }else if(this.$route.path.indexOf("/vegetableDevelopment") >= 0){
                auditRes = await vegetableDevelopmentAudit(ids,{status});  //农经股----蔬菜发展情况  审核
            }else if(this.$route.path.indexOf("/seedEnterprise") >= 0){
                ...
                auditRes = await diseaseControlDataAudit(ids,{status});  //植保站----疾病防治数据  审核
            }else if(this.$route.path.indexOf("/industryLeavePoor") >= 0){
        ...
        async downLoadTemplate(row){
            ...
            if(this.$route.path.indexOf("/qualityOrganicProducts") >= 0 || this.$route.path.indexOf("/doOrganicFoodEnter") >= 0){
               ...
               fileName = 'dsyz_product.xls';
                ...
                obj = {customTable:'breeding_cost'}; //畜牧股---养殖成本数据  下载模板传参
                fileName = 'breeding_cost.xls';
            }else if(this.$route.path.indexOf("/reformBusinessEntity") >= 0){ 
                ...
                obj = {customTable:'rattan_enterprise'}; //农经股--- 藤椒经营主体 下载模板传参
                fileName = 'rattan_enterprise.xls';
            }else if(this.$route.path.indexOf("/rpInformation") >= 0){ 
                obj = {customTable:'rattan_plant'}; //农经股--- 藤椒基本信息 下载模板传参
                fileName = 'rattan_plant.xls';
            }else if(this.$route.path.indexOf("/industrialDevelopment") >= 0){ 
                obj = {customTable:'rattan_development'}; //农经股--- 藤椒产业发展 下载模板传参
                fileName = 'rattan_development.xls';
            }else if(this.$route.path.indexOf("/vegetableDevelopment") >= 0){ 
                obj = {customTable:'vegetables_plant'}; //农经股--- 蔬菜发展情况 下载模板传参
                fileName = 'vegetables_plant.xls';
            }else if(this.$route.path.indexOf("/seedEnterprise") >= 0){ 
                ...
                obj = {customTable:'epidemic_surveillance'}; //植保站--- 监测点数据 下载模板传参
                fileName = 'epidemic_surveillance.xls';
            }else if(this.$route.path.indexOf("/diseaseControlData") >= 0){ 
                obj = {customTable:'pests_trends'}; //植保站--- 疾病防治数据 下载模板传参
                fileName = 'pests_trends.xls';
            }else if(this.$route.path.indexOf("/industryLeavePoor") >= 0){ 
